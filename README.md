...
3. If the first character of the command is a slash (`\`) or dot (`.`) or either of the above searches was successful, the shell executes the na```
...
#### exit
  * Usage: `exit [STATUS]`
  * Exits the shell.
  * The `STATUS` argument is the integer usedmed program with any remaining given arguments in a separate execution environment.
...
* Building a test suite to check our own code^C
$
```
